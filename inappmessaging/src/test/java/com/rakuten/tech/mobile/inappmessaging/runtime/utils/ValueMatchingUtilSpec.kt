...
class ValueMatchingUtilSpec : BaseTest() {
    ...
    fun `should compare integer yield correct result`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1, OperatorType.EQUALS, 1
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1, OperatorType.DOES_NOT_EQUAL, 1
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1, OperatorType.GREATER_THAN, 1
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1, OperatorType.LESS_THAN, 1
        ).shouldBeFalse()
    }
    ...
    fun `should compare double yield correct result`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.00001, OperatorType.EQUALS, 1.00001
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.00001, OperatorType.DOES_NOT_EQUAL, 1.00001
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.00001, OperatorType.GREATER_THAN, 1.00001
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.00001, OperatorType.LESS_THAN, 1.00001
        ).shouldBeFalse()
    }
    ...
    fun `should compare non time long yield true`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.EQUALS, LONG_VALUE, false
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.DOES_NOT_EQUAL, LONG_OTHER_VALUE, false
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_OTHER_VALUE, OperatorType.GREATER_THAN, LONG_VALUE, false
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.LESS_THAN, LONG_OTHER_VALUE, false
        ).shouldBeTrue()
    }
    ...
    fun `should compare non time long yield false`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.EQUALS, LONG_OTHER_VALUE, false
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.DOES_NOT_EQUAL, LONG_VALUE, false
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.GREATER_THAN, LONG_VALUE, false
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            LONG_VALUE, OperatorType.LESS_THAN, LONG_VALUE, false
        ).shouldBeFalse()
    }
    ...
    fun `should compare time long yield false`() {
        ...
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.DOES_NOT_EQUAL, TIME_DIFF, true
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.GREATER_THAN, TIME_DIFF, true
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.LESS_THAN, TIME_DIFF, true
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.GREATER_THAN, TIME_IN_MILLIS, true
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.LESS_THAN, TIME_IN_MILLIS, true
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TIME_IN_MILLIS, OperatorType.IS_BLANK, TIME_DIFF, true
        ).shouldBeFalse()
    }
    ...
    fun `should compare boolean yield correct result`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true, OperatorType.EQUALS, true
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true, OperatorType.DOES_NOT_EQUAL, true
        ).shouldBeFalse()
    }
    ...
    fun `should compare string yield correct result`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            "", OperatorType.EQUALS, TEST_STRING
        ).shouldBeFalse()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            "", OperatorType.DOES_NOT_EQUAL, TEST_STRING
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TEST_STRING, OperatorType.MATCHES_REGEX, TEST_STRING_REGEX
        ).shouldBeTrue()
        ValueMatchingUtil.isOperatorConditionSatisfied(
            TEST_STRING, OperatorType.DOES_NOT_MATCH_REGEX, TEST_STRING_REGEX
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare int with not match regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1,
            OperatorType.DOES_NOT_MATCH_REGEX, 1
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare double with regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.0,
            OperatorType.MATCHES_REGEX, 1.0
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare double with not match regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1.0,
            OperatorType.DOES_NOT_MATCH_REGEX, 1.0
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare long with regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1L,
            OperatorType.MATCHES_REGEX, 1L, false
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare long with not match regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            1L,
            OperatorType.DOES_NOT_MATCH_REGEX, 1L, false
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare boolean with regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true,
            OperatorType.MATCHES_REGEX, true
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare boolean with not match regex`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true,
            OperatorType.DOES_NOT_MATCH_REGEX, true
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare boolean with is not blank`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true,
            OperatorType.IS_NOT_BLANK, true
        ).shouldBeFalse()
    }
    ...
    fun `should be false when compare boolean with greater than`() {
        ValueMatchingUtil.isOperatorConditionSatisfied(
            true,
            OperatorType.GREATER_THAN, true
        ).shouldBeFalse()
    }
